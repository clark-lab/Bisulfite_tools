...
then #Regular job - the data should be there
  ...
  $QSUB -N "$PROJECT"_"$$"_prep_reads -wd "$PWD" -e "$PWD"/output -o "$PWD"/output -pe smp 4 -b y "$TOOLS"/prep_reads.sh "$1" "$FORWARD" "$REVERSE" &>> output/"$LOGFILE"
else #Spoon job - the data should appear after this hold_jid
  $QSUB -hold_jid $WAIT_JOB_ID -N "$PROJECT"_"$$"_prep_reads -wd "$PWD" -e "$PWD"/output -o "$PWD"/output -pe smp 4 -b y "$TOOLS"/prep_reads.sh "$1" "$FORWARD" "$REVERSE" &>> output/"$LOGFILE"
fi
